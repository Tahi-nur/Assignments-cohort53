// ! Function under test
export function addToShoppingCart(cart, item) {
  const newCart = [...cart, item]; // Add the new item using spread syntax

  // If more than 3 items, remove the first
  if (newCart.length > 3) {
    return newCart.slice(1); // Return array without the first item
  }

  return newCart; // Otherwise, return the full cart
}
